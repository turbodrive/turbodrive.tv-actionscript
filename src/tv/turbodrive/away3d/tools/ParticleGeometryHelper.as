package tv.turbodrive.away3d.tools
{
	...
	/**
	 * Personnal version for ParticleGeometryHelper. This one can work with non-CompactSubGeometry
	 */
			...
			for (i = 0; i < numParticles; i++)
			{
				...
				for (var srcIndex:int = 0; srcIndex < numSubGeometries; srcIndex++)
				{
					...
					else
					{
						...
						sourceVertices = sourceSubGeometry.vertexData;
						var sourceNormal:Vector.<Number> = sourceSubGeometry.vertexNormalData 
						var sourceTangents:Vector.<Number> = sourceSubGeometry.vertexTangentData
						var sourceUV:Vector.<Number> = sourceSubGeometry.UVData
						var uvIndex:int = 0 
						...
						for (k = 0; k < tempLen; k++)
						{
							...
							uvIndex = k * 2;
							//this is faster than that only push one data
							vertices.push(sourceVertices[product], sourceVertices[product + 1], sourceVertices[product + 2],
								sourceNormal[product], sourceNormal[product + 1], sourceNormal[product + 2],
								sourceTangents[product], sourceTangents[product + 1], sourceTangents[product + 2],
								sourceUV[uvIndex], sourceUV[uvIndex + 1],
								sourceUV[uvIndex], sourceUV[uvIndex + 1]);
						}
